...
// フィールド:
//   - status        : "offline" | "online" | "ingame" | "reconnecting"
//   - match_id      : 参戦中の試合ID（ingame時のみ値あり）
//   - updated_at    : UNIXミリ秒（心拍/状態更新時に上書き）
//   - socket_count  : WS接続数（多タブ考慮）
//
// TTLポリシー: すべての更新で PEXPIRE 30,000ms をセット（心拍で延長）。
//
